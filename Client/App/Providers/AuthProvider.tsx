...
export default function AuthProvider ({children}:PropsWithChildren){
        ...
        useEffect(()=>{
            const fetchUsers=async()=>{
                const res = await axios.get(`${url}/email/${user?.emailAddresses[0].emailAddress}`)
    ...
    return(
        <AuthContext.Provider value={{Login,Register,RegisterVerification, LoginVerification, url, userData}}>
            {children}
